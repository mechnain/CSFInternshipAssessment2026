...
Out of 300 queries, 99 (33%) ended up in the failure visualization. The visualization shows two distinct mechanisms.

**Failure mode 1: same-coat confusion among dark-coated dogs.** Several queries of `dog_1234` (wiry black terrier mix) are confidently mis-matched to other dark-coated identities (`dog_646`, `dog_493`, `dog_895`) at similarities 0.71-0.75. The rank-1 decision is `match`, just to the wrong individual. The true identity is often present at rank 2 or 3 with a similarity within 0.02 of rank 1, which is the Rank-1 / Rank-5 gap visible in the metrics. Mitigations: (a) per-image gallery retrieval (max over reference images) instead of a single prototype, which would preserve a high-similarity view that the average smooths away, (b) re-ranking with a more expensive pairwise comparison on the top-k candidates, (c) a small metric-learning fine-tune that explicitly separates within-coat identities.

**Failure mode 2: open-set queries land in the abstention band, not in `unknown`.** Most failures for true-unknown queries are `ambiguous_unknown`: a light-coated dog scores 0.56-0.68 against the closest gallery prototype, which puts it between `tau_possible=0.55` and `tau_match=0.70` so the system abstains rather than rejects. This is consistent with the metric picture: unknown_accuracy=0.100 is low, but non_match_rejection=0.800 is high, meaning the system rarely commits to a wrong identity for an absent dog; it just declines to issue a hard `unknown`. Mitigations: (a) calibrate `tau_possible` on a validation set with the operating cost of human review weighed against the cost of a false alarm, (b) add an explicit known-vs-unknown classifier on top of the top-1 similarity, exploiting the 0.871 AUROC signal that the embedding space already provides, (c) per-identity prototype variance as an extra feature, since well-represented identities should have tighter similarity distributions than under-represented ones.

A third pattern - global embeddings confusing background and pose with identity - is visible in the success visualization too (the same backdrop appears in rank-1 hits for several queries). Detection or segmentation before embedding would reduce that signal.
